import { Award, BookOpen, Star, User } from "lucide-react";
      ...
      <div className="absolute bottom-6 left-8 right-8 flex items-end justify-between">
        <div className="flex items-end space-x-6">
          <div className="relative">
            <div className="w-32 h-32 rounded-2xl bg-white p-2 shadow-xl overflow-hidden">
              {student.profilePhoto ? (
                <img src={student.profilePhoto} alt={userInfo.name || "Profile"} className="w-full h-full object-cover rounded-xl" />
              ) : (
          ...
          <div className="text-white pb-4 font-[${Poppins.style.fontFamily}]">
            <h2 className="text-2xl font-bold mb-2">{userInfo.name || "Student"}</h2>
            <div className="flex items-center space-x-4 text-white/90">
              <div className="flex items-center space-x-1">
                <Star className="w-4 h-4 fill-yellow-400 text-yellow-400" />
                <span className="font-semibold">{student.averageScore || 0}% Avg Score</span>
              </div>
              <div className="flex items-center space-x-1">
            ...
            {userInfo.bio && userInfo.bio !== "N/A" && (
              <p className="text-white/80 text-sm mt-2 max-w-md">{userInfo.bio}</p>
            )}
          </div>
        </div>
