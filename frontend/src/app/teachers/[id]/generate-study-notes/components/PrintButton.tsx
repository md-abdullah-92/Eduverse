...
export default function SaveSlideButton({ title }: SaveSlideButtonProps) {
  ...
  const getSanitizedClone = (): HTMLElement => {
    ...
    if (!preview) {
      toast.error("Slide preview not found");
    ...
    clone.querySelectorAll<HTMLElement>("*").forEach((el) => {
      ...
      el.style.color = "#111827"; // Darker for readability
    });

    // Remove first h2 if needed
  ...
  const handlePrint = async () => {
    try {
      ...
      const html = `
        ...
        <html>
          <head>
            <title>${title || "Teaching Notes"}</title>
  ...
  return (
    <div className="flex gap-4">
      ...
      >
        ...
        {isLoading ? "Processing..." : "Print Notes"}
      </Button>
    </div>
  );
}
