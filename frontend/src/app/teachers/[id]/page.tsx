  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        ...
        localStorage.setItem(
          "role",
          data.teacherProfile.user.role || "TEACHER"
        );
        localStorage.setItem(
          "userEmail",
          data.teacherProfile.user.email || ")"
        );
        localStorage.setItem(
          "userPhone",
      ...
      <main className="flex-1 p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <StatCard
            ...
            value={mentor.totalStudents?.toString() || "-"}
            icon={<Users />}
            color="text-blue-500"
          />
          <StatCard
            label="Total Sales"
            value={mentor.totalSales ? `$${mentor.totalSales}` : "-"}
            icon={<DollarSign />}
            color="text-green-500"
          />
          <StatCard
            label="Total Courses"
            value={mentor.totalCourses?.toString() || "-"}
            icon={<BookOpen />}
            color="text-cyan-500"
          />
          <StatCard
            label="Processing Orders"
            value={mentor.processingOrders?.toString() || "-"}
            icon={<ClipboardList />}
            color="text-purple-500"
          />
          <StatCard
            label="Completed Orders"
            value={mentor.completedOrders?.toString() || "-"}
            icon={<ClipboardList />}
            color="text-pink-500"
          />
          <StatCard
            label="Total Orders"
            value={mentor.totalOrders?.toString() || "-"}
            icon={<ClipboardList />}
        ...
        <div className="bg-white rounded-xl p-6 shadow space-y-3">
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-gray-500">
                4 Lessons · 1,200 Students · 3 Weeks
              </p>
